...
public static class State
{
    ...
    public static StateStore LoadState(string path)
    {
        ...
        try
        {
            ...
            if (store.Version > 1)
            {
                Notifier.LogError($"Warning: state file version {store.Version} is newer than supported (1), starting fresh");
                return new StateStore();
            }
    ...
    public static void SaveState(string path, StateStore store)
    {
        ...
        catch (Exception ex) { Notifier.LogError($"Failed to save state: {ex.Message}"); }
    }
