...
1. **Check URLs from CLI** — Project skeleton + config + HTTP checker + single-round CLI. Verify: `url-monitor https://example.com` prints a check result.
...
3. **Continuous monitoring** — Poll loop + logger + graceful shutdown. Verify: tool runs continuously, logs to file, exits cleanly on Ctrl+C.
