...
**You are expected to use an AI coding agent (GitHub Copilot, Claude Code, Cursor, or similar) throughout this exercise.** The goal is to practice collaborating with an AI assistant on a complex domain modeling task.
...
This domain is a natural fit for C# features. Consider using:

- **Records** for immutable domain entities (ECU, TestCase, TestResult)
- **Enums** for fixed classifications (EcuType, BenchStatus, Verdict)
- **Interfaces** for component boundaries (IOrchestrationEngine, ISignalGenerator, IResultsRepository)
- **Result types** or custom exception hierarchies for error handling
- **Strongly-typed IDs** (e.g., `record TestCaseId(string Value)`) instead of raw strings
- **Nullable reference types** to express optional fields
- **LINQ** for querying the results repository

### Getting Started

```bash
dotnet new console -n TestBenchSystem
cd TestBenchSystem
```

Or for a more structured project:

```bash
dotnet new sln -n TestBenchSystem
dotnet new classlib -n TestBenchSystem.Core -o src/TestBenchSystem.Core
dotnet new console -n TestBenchSystem.Cli -o src/TestBenchSystem.Cli
dotnet new xunit -n TestBenchSystem.Tests -o tests/TestBenchSystem.Tests
dotnet sln add src/TestBenchSystem.Core src/TestBenchSystem.Cli tests/TestBenchSystem.Tests
```
