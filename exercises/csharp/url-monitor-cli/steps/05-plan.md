...
This means you can **spot-check** the plan rather than performing a deep line-by-line review. If the design said "use HttpClient with async requests" and the structure said "Phase 1 creates Checker.cs," the plan just fills in the implementation details for that checker. The strategic decisions are already settled.
...
- Test cases to write
- Verification commands (`dotnet test`, manual checks)
...
- **Are verification steps runnable?** Each phase should end with specific `dotnet test` commands and manual checks you can actually execute.
- **Is it self-contained?** Could someone read just this plan and implement the feature without referencing the other documents?
- **Are the test cases specific?** "Test the checker" is too vague. "Test that a 500 response produces a CheckResult with Success==false and StatusCode==500" is concrete.
...
- [ ] File paths are specific (not "create a module for X" but "create `src/Checker.cs` and `src/Models/CheckResult.cs`")
...
- **Vague verification.** "Verify it works" is not a verification step. "Run `dotnet test --logger "console;verbosity=detailed"` and confirm 4 tests pass" is.
- **Missing test cases.** Every new class or method should have tests specified in the plan.
