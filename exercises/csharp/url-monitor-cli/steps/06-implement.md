...
4. Run verification: `dotnet build && dotnet test` and any manual checks
...
dotnet build
dotnet run -- --help
dotnet run -- https://httpbin.org/status/200    # should print check result to stderr
...
# After implementation:
dotnet build
dotnet test                                      # Phase 1 + Phase 2 tests pass
...
# After implementation:
dotnet build
dotnet run -- https://httpbin.org/status/200 --interval 5    # runs continuously
# Ctrl+C exits cleanly
dotnet run -- https://example.com --log-file monitor.log     # logs to file
...
- [ ] The tool works manually: `dotnet run -- <url>` produces expected output
