...
- **Idiomatic C#** -- Does the design use idiomatic C# patterns (dependency injection, async/await, records)? Or is it falling back to procedural patterns that don't fit the .NET ecosystem?
- **Data types** -- CheckResult, UrlState, Transition -- would records work well here? Are the types leveraging .NET's type system effectively?
...
- **Built-in features** -- Does the design leverage built-in .NET features (HttpClient, IHostedService, CancellationToken) or is it reinventing the wheel?
...
- **Dependencies** -- Are the chosen NuGet packages appropriate? Is the dependency management strategy clear?
