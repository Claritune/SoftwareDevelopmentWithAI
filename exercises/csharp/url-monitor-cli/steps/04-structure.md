...
The natural tendency -- for both humans and AI agents -- is to plan horizontally: "first build all the models, then all the services, then all the tests." This feels organized but has a critical flaw: nothing works until everything is assembled. You can't test the state machine until the checker exists. You can't test notifications until the state machine exists. If something is wrong in the models layer, you don't find out until you wire up the services three steps later.
...
Think of the structure outline like a C# interface definition -- it defines the contracts and types without implementing them. It shows WHAT changes in each slice, not HOW (that's the plan's job).
...
1. **Check URLs from CLI** -- .NET project setup + HttpClient checker + single-round CLI. Verify: `dotnet run -- https://example.com` prints a check result.
...
3. **Continuous monitoring** -- Poll loop + logging + graceful shutdown (CancellationToken / Console.CancelKeyPress). Verify: tool runs continuously, logs to file, exits cleanly on Ctrl+C.
...
- **Horizontal slicing.** "Phase 1: all models. Phase 2: all services." Each phase should produce testable behavior, not just code artifacts.
