...
2. **Assess project maturity** with light exploration (codebase-locator agent or quick scan):
   - **Greenfield**: No application runtime code -- no `src/`, entry points (`Program.cs`, `*.cs`), domain modules, application tests, or dependency manifests tied to implementation (`.csproj`, `.sln`). May contain only goal docs, coding rules, and workflow skills.
