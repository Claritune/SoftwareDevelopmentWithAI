...
2. **Assess project maturity** with light exploration (codebase-locator agent or quick scan):
   - **Greenfield**: No application runtime code -- no `src/`, `app/`, entry points (`Program.cs`, `Main()` method), domain modules, application tests, or dependency manifests tied to implementation. May contain only goal docs, coding rules, and workflow skills.
...
Good: "How does the middleware pipeline handle request authentication, and where are auth policies defined?"
