---
description: Expands the structure outline into a detailed tactical implementation plan for the agent. Use when running the QRSPI Plan phase with structure.md, design.md, and research.md, or when the user invokes /plan.
model: opus
...
2. **For each phase in `structure.md`**, expand into full implementation detail:
   ...
   - Code snippets for non-trivial changes (new functions, type definitions, class declarations)
...
3. **Write `plan.md`** to the artifact directory:
   ...
   **Action**: [create / modify / delete]
   ...
   ### Verification
   #### Automated
   - [ ] `cmake --build build` compiles without errors or warnings
...
- Use the project's existing test/lint/build commands for verification. Check CLAUDE.md, Makefile, or CMakeLists.txt for the right commands.
