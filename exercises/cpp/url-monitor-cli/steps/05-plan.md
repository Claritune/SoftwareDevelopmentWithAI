...
This means you can **spot-check** the plan rather than performing a deep line-by-line review. If the design said "use cpp-httplib with synchronous requests" and the structure said "Phase 1 creates checker.cpp," the plan just fills in the implementation details for that checker. The strategic decisions are already settled.
...
- Test cases to write
- Verification commands (`ctest`, manual checks)
...
- **Are verification steps runnable?** Each phase should end with specific `ctest` commands and manual checks you can actually execute.
...
- [ ] File paths are specific (not "create a module for X" but "create `src/checker.cpp` and `include/url_monitor/checker.h`")
...
- **Vague verification.** "Verify it works" is not a verification step. "Run `ctest --test-dir build --output-on-failure` and confirm 4 tests pass" is.
