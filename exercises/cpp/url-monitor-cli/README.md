...
A C++ CLI tool that monitors URLs for uptime:
...
├── include/                <- header files (empty until Step 6)
├── tests/                  <- test code (empty until Step 6)
├── CMakeLists.txt          <- build configuration (empty until Step 6)
