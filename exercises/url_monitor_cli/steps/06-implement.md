...
4. Run verification: `pytest tests/ -v` and any manual checks
...
pip install -e ".[dev]"
url-monitor --help
url-monitor https://httpbin.org/status/200    # should print check result to stderr
...
```bash
# After implementation:
pytest tests/ -v                               # Phase 1 + Phase 2 tests pass
...
```bash
# After implementation:
url-monitor https://httpbin.org/status/200 --interval 5    # runs continuously
# Ctrl+C exits cleanly
url-monitor https://example.com --log-file monitor.log     # logs to file
...
- [ ] The tool works manually: `url-monitor <url>` produces expected output
